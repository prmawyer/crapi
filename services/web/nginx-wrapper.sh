...
ls -al /app/certs
env
envsubst '${HTTP_PROTOCOL} ${COMMUNITY_SERVICE} ${IDENTITY_SERVICE} ${WORKSHOP_SERVICE} ${MAILHOG_WEB_SERVICE} ${TRACEABLE_CAPTCHA_SITE_KEY_FOR_LOGIN_FLOW} ${TRACEABLE_CAPTCHA_URL}' < $NGINX_TEMPLATE > /etc/nginx/conf.d/default.conf
openresty
exec "$@"